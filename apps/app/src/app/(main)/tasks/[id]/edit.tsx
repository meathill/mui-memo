...
const PLACES: { value: TaskPlace; label: string }[] = SHARED_PLACES.map(
	(value) => ({
		value,
		label: PLACE_LABEL[value].label,
	}),
);
const WINDOWS: { value: TaskWindow; label: string }[] = SHARED_WINDOWS.map(
	(value) => ({
		value,
...
function toFreqInterval(
	repeat: RepeatOption,
): { freq: RecurrenceFreq; interval: number } | null {
	switch (repeat) {
		case "daily":
...
export default function TaskEditScreen() {
	...
	const [loadedRecurrence, setLoadedRecurrence] =
		useState<RecurrenceInfo | null>(null);
	...
	function fillForm(nextTask: TaskView, recurrence: RecurrenceInfo | null) {
		...
		setRepeat(
			recurrence ? toRepeat(recurrence.freq, recurrence.interval) : "none",
		);
	}

	// biome-ignore lint/correctness/useExhaustiveDependencies: legacy dependencies
	useEffect(() => {
		if (!id) return;
		let cancelled = false;
		(async () => {
	...
	const handleSave = useCallback(async () => {
		...
		const finalTags =
			draft && !tags.includes(draft) && tags.length < 12
				? [...tags, draft]
				: tags;
		if (JSON.stringify(finalTags) !== JSON.stringify(task.tags ?? []))
		...
		const recurrenceChanged =
			(desired?.freq ?? null) !== currentFreq ||
			(desired?.interval ?? null) !== currentInterval;
		...
		function buildRecurrenceInput(spec: {
			freq: RecurrenceFreq;
			interval: number;
		}): RecurrenceInput {
			return {
		...
		try {
			if (taskChanged) {
				const next = await patchTaskEverywhere(
					task.id,
					taskPatchToLocalPatch(patch),
				);
							...
							{tags.length > 0 ? (
								<View className="mb-2 flex-row flex-wrap gap-2">
									{tags.map((t) => (
										<View
											key={t}
											className="flex-row items-center gap-1 rounded-full bg-ink py-1.5 pr-2 pl-3"
										>
											<Pressable
												onPress={() => editTag(t)}
												hitSlop={4}
												accessibilityLabel={`编辑标签 ${t}`}
											>
