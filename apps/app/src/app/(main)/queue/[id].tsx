			...
			{item ? (
				<View className="px-5 pt-2">
					<View className="rounded-2xl border border-rule/60 bg-paper-2/40 p-5">
						...
						<Text className="mt-3 font-serif text-2xl text-ink">
							语音 · {formatDuration(item.durationMs)}
						</Text>
						<Text className="mt-1 font-mono text-ink-mute text-xs">
					...
					{item.status === "processing" ? (
						<Text className="mt-4 text-center text-ink-mute text-sm">
							正在处理中，处理完会自动变成任务
						</Text>
					) : (
...
function StatusLine({ status, error }: { status: string; error?: string }) {
	if (status === "processing") {
		return (
			<Text className="font-mono text-accent-warm text-xs uppercase tracking-[2px]">
				处理中
			</Text>
		);
	}
	if (status === "error") {
		return (
			<View>
				<Text className="font-mono text-accent-warn text-xs uppercase tracking-[2px]">
					处理失败
				</Text>
				{error ? (
					<Text className="mt-1 text-accent-warn text-sm">{error}</Text>
				) : null}
			</View>
		);
	}
