...
import { type LocalTaskPatch, mergeTaskPatch } from "@/lib/local-cache-model";
...
export async function loadCachedTasks(): Promise<TaskView[]> {
	...
	const rows = await db.getAllAsync<JsonRow>(
		"SELECT json FROM local_tasks ORDER BY cached_order ASC",
	);
	return rows
		.map((row) => parseJson<TaskView>(row.json))
...
export async function loadCachedCompletedFirstPage(): Promise<CompletedTask[]> {
	const db = await initLocalDb();
	const rows = await db.getAllAsync<JsonRow>(
		"SELECT json FROM local_completed_tasks ORDER BY cached_order ASC",
	);
	return rows
		.map((row) => parseJson<CompletedTask>(row.json))
		.filter((task): task is CompletedTask => Boolean(task));
}

export async function replaceCachedCompletedFirstPage(
	tasks: CompletedTask[],
): Promise<void> {
	const db = await initLocalDb();
	const now = new Date().toISOString();
...
export async function loadCachedTaskDetail(
	id: string,
): Promise<CachedTaskDetail | null> {
	const db = await initLocalDb();
	const row = await db.getFirstAsync<JsonRow>(
		"SELECT json FROM local_task_details WHERE id = ?",
		id,
	);
	return row ? parseJson<CachedTaskDetail>(row.json) : null;
}

export async function saveCachedTaskDetail(
...
export async function applyLocalTaskPatch(
	id: string,
	patch: LocalTaskPatch,
): Promise<TaskView | null> {
	const db = await initLocalDb();
	const row = await db.getFirstAsync<JsonRow>(
		"SELECT json FROM local_tasks WHERE id = ?",
		id,
	);
	...
	await db.withTransactionAsync(async () => {
		await db.runAsync(
			"UPDATE local_tasks SET json = ?, updated_at = ? WHERE id = ?",
			JSON.stringify(next),
			now,
			id,
		);
		...
		const detailRow = await db.getFirstAsync<JsonRow>(
			"SELECT json FROM local_task_details WHERE id = ?",
			id,
		);
		const detail = detailRow
			? parseJson<CachedTaskDetail>(detailRow.json)
			: null;
		if (detail) {
			await db.runAsync(
...
async function setSyncMetaInDb(
...
async function upsertCachedTaskInDb(
	db: SQLite.SQLiteDatabase,
	task: TaskView,
	now: string,
): Promise<void> {
	const row = await db.getFirstAsync<OrderRow>(
		"SELECT cached_order FROM local_tasks WHERE id = ?",
		task.id,
	);
	await db.runAsync(
