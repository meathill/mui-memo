...
describe('legacy AsyncStorage 任务迁移解析', () => {
  it('从 zustand persist JSON 里取出旧 tasks', () => {
    ...
    expect(extractLegacyTasksFromPersistedState(stored).map((item) => item.id)).toEqual(['a', 'b']);
  });

  it('坏 JSON、缺 tasks、非法 task 都安全返回空数组或过滤掉', () => {
    expect(extractLegacyTasksFromPersistedState('{')).toEqual([]);
    expect(extractLegacyTasksFromPersistedState(JSON.stringify({ state: { theme: 'paper' } }))).toEqual([]);
    expect(
      extractLegacyTasksFromPersistedState(JSON.stringify({ state: { tasks: [{ id: 'x' }, task({ id: 'ok' })] } })),
    ).toEqual([task({ id: 'ok' })]);
  });
  ...
  it('stripLegacyTasksFromPersistedState 只删除 tasks，保留其它轻量状态', () => {
    ...
    const parsed = JSON.parse(stripped ?? '{}') as { state: Record<string, unknown> };
...
describe('本地标签候选', () => {
  ...
  it('最多收集 limit 个', () => {
    const tasks = Array.from({ length: 105 }, (_, i) => task({ id: `t${i}`, tags: [`标签${i}`] }));
...
describe('本地任务 patch', () => {
  it('done patch 会设置 done=true 和 completedAt', () => {
    const next = mergeTaskPatch(task(), { status: 'done' }, new Date('2026-06-24T04:00:00.000Z'));
  ...
  it('pending patch 会清 completedAt', () => {
    const next = mergeTaskPatch(task({ status: 'done', done: true, completedAt: '2026-06-24T04:00:00.000Z' }), {
      status: 'pending',
    });
  ...
  it('taskPatchToLocalPatch 保留显式 null，用于清空时间字段', () => {
    expect(taskPatchToLocalPatch({ expectAt: null, dueAt: null, tags: [] })).toEqual({
      expectAt: null,
      dueAt: null,
      tags: [],
    });
  ...
  it('completedTaskToView 生成可回滚的任务视图', () => {
    expect(
      completedTaskToView({ id: 'c1', text: '交物业费', tags: ['财务'], completedAt: '2026-06-24T04:00:00Z' }),
    ).toMatchObject({
      id: 'c1',
      status: 'done',
      done: true,
...
describe('刷新 TTL', () => {
  it('强制刷新、空任务、无同步时间都会刷新', () => {
    expect(shouldRefreshTasks({ force: true, tasksLength: 10, lastTasksSyncedAt: 1, now: 2 })).toBe(true);
    expect(shouldRefreshTasks({ tasksLength: 0, lastTasksSyncedAt: 1, now: 2 })).toBe(true);
    expect(shouldRefreshTasks({ tasksLength: 10, lastTasksSyncedAt: null, now: 2 })).toBe(true);
  });

  it('有缓存且未超过 TTL 时不刷新，超过 TTL 时刷新', () => {
    expect(shouldRefreshTasks({ tasksLength: 10, lastTasksSyncedAt: 1_000, now: 30_000, ttlMs: 60_000 })).toBe(false);
    expect(shouldRefreshTasks({ tasksLength: 10, lastTasksSyncedAt: 1_000, now: 61_000, ttlMs: 60_000 })).toBe(true);
  });
});
