...
export async function applyConfirm(
	...
	if (effect.kind === "modify") {
		const body =
			choice === "confirm"
				? {
						...
						taskId: effect.id,
						patch: effect.patch as Record<string, unknown>,
					}
				: {
						kind: "modify-as-add" as const,
						rawText: utterance.raw,
						task: { ...effect.before, ...effect.patch } as Record<
							string,
							unknown
						>,
		...
		const { tasks } = await api.intent.confirm({
			kind: "done",
			taskId: effect.id,
		});
		await replaceTasksEverywhere(tasks);
