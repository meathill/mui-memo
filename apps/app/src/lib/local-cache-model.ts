...
export type LocalTaskPatch = Partial<
  Pick<TaskView, 'text' | 'place' | 'energy' | 'priority' | 'tags' | 'deadline' | 'expectAt' | 'dueAt'>
> & {
  window?: TaskWindow;
  status?: TaskStatus;
...
export function extractLegacyTasksFromPersistedState(value: string | null): TaskView[] {
  if (!value) return [];
  try {
    const parsed = JSON.parse(value) as PersistedAppState;
    return Array.isArray(parsed.state?.tasks) ? parsed.state.tasks.filter(isTaskViewLike) : [];
  } catch {
    return [];
  }
}

export function stripLegacyTasksFromPersistedState(value: string | null): string | null {
...
export function collectLocalTagCandidates(tasks: TaskView[], limit = 100): string[] {
  const out: string[] = [];
...
export function mergeTaskPatch(task: TaskView, patch: LocalTaskPatch, now = new Date()): TaskView {
  ...
  if (patch.status !== undefined) {
    ...
    next.completedAt = patch.status === 'done' ? (patch.completedAt ?? next.completedAt ?? now.toISOString()) : null;
...
export function taskPatchToLocalPatch(patch: Partial<TaskPatch>): LocalTaskPatch {
...
export function shouldRefreshTasks(args: {
  ...
  return args.now - args.lastTasksSyncedAt >= (args.ttlMs ?? TASKS_REFRESH_TTL_MS);
}
...
export function shouldClearTaskSnapshotForUserChange(args: {
  ...
  return Boolean(args.previousUserId && args.previousUserId !== args.nextUserId);
}
