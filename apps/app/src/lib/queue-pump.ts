...
async function tick(): Promise<void> {
	...
	if (stale.length > 0) {
		for (const q of stale)
			useAppStore.getState().updateQueueItem(q.id, { status: "queued" });
	...
	try {
		const { utterance, effects, tasks, pendingConfirms } =
			await api.intent.submit({
		...
		if (pendingConfirms?.length) {
			useAppStore.getState().pushPendingConfirms(
...
export function startQueuePump(): () => void {
	...
	const unsub = useAppStore.subscribe((s, prev) => {
		if (s.queue === prev.queue && s.pendingConfirms === prev.pendingConfirms)
			return;
		void tick();
	});
