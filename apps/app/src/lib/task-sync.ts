...
import { syncTodayTasksWidgetSnapshot } from "@/widgets/today-tasks-widget-sync";
...
export async function hydrateTasksFromLocalCache(): Promise<TaskView[]> {
	...
	if (syncedAt) {
		useAppStore
			.getState()
			.setTasksSyncState({ lastTasksSyncedAt: Date.parse(syncedAt) || null });
	}
	await syncTodayTasksWidgetSnapshot(tasks);
	return tasks;
}
...
export async function replaceTasksEverywhere(tasks: TaskView[]): Promise<void> {
	...
	useAppStore
		.getState()
		.setTasksSyncState({ lastTasksSyncedAt: Date.now(), tasksSyncError: null });
	await syncTodayTasksWidgetSnapshot(tasks);
}

export async function refreshTasksFromRemote(
	opts: { force?: boolean } = {},
): Promise<TaskView[]> {
...
export async function patchTaskEverywhere(
	id: string,
	...
	if (next) {
		...
		const tasks = current.map((task) => (task.id === id ? next : task));
...
export async function reopenCompletedTaskLocally(
	task: CompletedTask,
): Promise<void> {
	await removeCachedCompletedTask(task.id);
	const view = completedTaskToView(task);
	await restoreTaskEverywhere({
		...view,
		status: "pending",
		done: false,
		completedAt: null,
...
export async function migrateLegacyTasksFromAsyncStorage(): Promise<
	TaskView[]
> {
	...
	if (!cached.length) {
		...
		await syncTodayTasksWidgetSnapshot(legacyTasks);
	}
