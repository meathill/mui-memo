...
export function AudioPlayButton({ audioKey, label = "播放原声" }: Props) {
	...
	return (
		...
		>
			{playing ? (
				<PauseIcon size={16} color={colors.accentWarm} />
			) : (
				<PlayIcon size={16} color={colors.accentWarm} />
			)}
...
export function LocalAudioPlayButton({
	uri,
	label = "试听录音",
}: {
	uri: string;
...
function useAudioErrorFallback(status: AudioStatus): {
	error: boolean;
	onPlay: () => void;
} {
