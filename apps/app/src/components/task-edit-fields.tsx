...
export function ExpectAtField({ value, onChange }: { value: string | null; onChange: (iso: string | null) => void }) {
  ...
  return (
    <>
      ...
      <Pressable onPress={handleOpenPicker} className="mt-3 flex-row items-center gap-1.5 self-start py-1" hitSlop={8}>
        <CalendarIcon size={14} color={value ? colors.ink : colors.inkMute} />
        <Text className={`font-mono text-xs ${value ? 'text-ink underline' : 'text-ink-mute'}`}>
