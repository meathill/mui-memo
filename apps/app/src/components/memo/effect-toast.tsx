...
export function EffectToast({ effects, utterance }: Props) {
	...
	return (
		<View className="mt-3 gap-1.5">
			{list.map((effect, i) => {
				const tone =
					effect.kind === "miss" ? "bg-accent-warn" : "bg-accent-good";
				return (
					<View
					...
					>
						<Text className="font-medium text-paper text-base">
							{effect.verb ?? effect.kind}
						</Text>
			...
			{more > 0 ? (
				<Text className="text-center text-ink-soft text-xs">
					还有 {more} 条
				</Text>
			) : null}
		</View>
