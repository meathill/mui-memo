...
const PLACE_CANDIDATES: BarChip[] = PLACES.map((place) => ({
	kind: "place",
	place,
}));
 ...
 */
export function BarEditorModal({
	visible,
	chips,
	allTags,
	onSave,
	...
	const addable = useMemo<BarChip[]>(() => {
		...
		return [...PLACE_CANDIDATES, ...tagCandidates].filter(
			(c) => !draft.some((d) => sameChip(d, c)),
		);
	}, [allTags, draft]);
	...
	function handleSave() {
		...
		const next = draft.some((d) => sameChip(d, PINNED))
			? draft
			: [...draft, PINNED];
		onSave(next);
	}
				...
				<View className="flex-1 justify-end bg-black/40">
					<View className="rounded-t-2xl bg-paper px-4 pt-4 pb-8">
						<View className="mb-4 flex-row items-center justify-between">
							...
							<Text className="font-serif font-bold text-base text-ink">
								筛选栏
							</Text>
						...
						>
							...
							{addable.length > 0 ? (
								<>
									<Text className="mt-5 mb-2 font-mono text-ink-mute text-xs uppercase tracking-[2px]">
										可添加
									</Text>
							...
							{allTags.length === 0 ? (
								<Text className="mt-4 px-1 text-ink-mute text-xs">
									还没有标签。给任务加上标签后，这里就能选了。
								</Text>
							) : null}
						</ScrollView>
					</View>
