...
export function TaskRow({ task, onDone, onReopen }: Props) {
	...
	const containerStyle = useAnimatedStyle(() => ({
		...
		height:
			collapse.value < 1 ? naturalHeight.value * collapse.value : undefined,
		overflow: "hidden",
	}));
		...
		>
			...
			>
				{isDoneRecurring ? (
					...
					>
						<Animated.View
							style={[StyleSheet.absoluteFill, styles.iconCenter, circleStyle]}
							pointerEvents="none"
						>
				...
				<View className="flex-1">
					<View style={styles.relative}>
						<View className="flex-row items-center gap-1.5">
							{task.recurrenceId ? (
								<RepeatIcon size={13} color={colors.inkMute} />
							) : null}
							<Text
