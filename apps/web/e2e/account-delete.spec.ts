...
test.describe('注销账号', () => {
  test('注销后账号与会话被永久删除', async ({ playwright, baseURL }) => {
    ...
    expect(signUp.status(), await signUp.text().catch(() => '')).toBeLessThan(400);
    ...
    const reLogin = await ctx.post('/api/auth/sign-in/email', { data: { email, password } });
    expect(reLogin.status(), '已注销账号不应能再登录').toBeGreaterThanOrEqual(400);
