...
const TINY_WEBM = Buffer.from([
  ...
  0x42, 0xf2, 0x81, 0x04, 0x42, 0xf3, 0x81, 0x08, 0x42, 0x82, 0x88, 0x6d, 0x61,
  0x74, 0x72, 0x6f, 0x73, 0x6b, 0x61, 0x42, 0x87, 0x81, 0x04, 0x42, 0x85, 0x81,
  0x02,
]);
...
test.describe("音频归档回放", () => {
  ...
  test("有 audioKey 的任务 → 详情页出现 <audio> → /api/audio 流 200 + Range", async ({
    inject,
    page,
  }) => {
    // 1) 先注一条任务
    ...
    const open = await page.request.get(`/api/audio/${key}`, {
      headers: { Range: "bytes=0-" },
    });
    expect(open.status()).toBe(206);
    expect(open.headers()["content-range"]).toBe(
      `bytes 0-${TINY_WEBM.length - 1}/${TINY_WEBM.length}`,
    );
    ...
    // 8) HEAD → 200 头 + 无 body
    const head = await page.request.fetch(`/api/audio/${key}`, {
      method: "HEAD",
    });
  ...
  test("访问别人的 audio key → 403", async ({ page }) => {
    const res = await page.request.get(
      "/api/audio/muimemo/audio/someone-else/any.webm",
    );
