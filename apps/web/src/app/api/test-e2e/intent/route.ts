import { applyActions, type IntentEffect, rerank } from '@mui-memo/shared/logic';
import { type Action, parseUtteranceFlexible, taskPlaceEnum, type Utterance } from '@mui-memo/shared/validators';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'invalid utterance', issues }, { status: 400 });
  }
    ...
    await Promise.all(
      utterance.actions.map(async (action) => {
        ...
        try {
          ...
          const resolved = await resolveTargetTask(ctx.db, userId, query, action.match);
  ...
  await logUtterance(ctx.db, userId, utterance, effects, null).catch(() => undefined);
  ...
  return NextResponse.json({ utterance, effect, effects, tasks: tasksAfter, ranked });
}
