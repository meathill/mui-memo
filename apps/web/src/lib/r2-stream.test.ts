...
describe("resolveRange", () => {
  it("start-end 转 R2Range", () => {
    expect(resolveRange({ start: 0, end: 1 }, 100)).toEqual({
      offset: 0,
      length: 2,
    });
  });
  ...
  it("start-end 末尾 clamp 到 size-1", () => {
    expect(resolveRange({ start: 90, end: 200 }, 100)).toEqual({
      offset: 90,
      length: 10,
    });
  });
...
function makeBucket(
  content: Uint8Array,
  contentType: string | undefined = "audio/mp4",
): R2Bucket {
  const headObj = {
...
function makeRequest(opts?: { method?: string; range?: string }): Request {
  ...
  return new Request("http://test/audio", {
