...
export const CASES: PromptEvalCase[] = [
  // ─── 回归 v0.8 的误判 bug ───────────────────────────
  {
    ...
    text: '完了转需要提供快递单号',
    currentTasks: [{ id: 'a', text: '提供货单给高老师', place: 'work', window: 'today' }],
    expected: [{ intent: 'ADD', taskTextContains: '快递' }],
    comment: 'v0.8 时 AI 误判 MODIFY 把第一条 text 改成了"提供快递单号给高老师"',
  },
  {
    name: '过渡词「另外」开头：新增不是修改',
    text: '另外明天买菜',
    currentTasks: [{ id: 'a', text: '交物业费', place: 'home', window: 'today' }],
  ...
  // ─── 显式 MODIFY 信号 ────────────────────────────────
  {
    ...
    expected: [{ intent: 'MODIFY', matchContains: '买菜', patchHasExpectAt: true }],
  },
  ...
  // ─── STATUS / DONE / LINK ─────────────────────────────
  {
    ...
    text: '我现在开始写报告',
    currentTasks: [{ id: 'a', text: '写月度报告', place: 'work', window: 'today' }],
    expected: [{ intent: 'STATUS', matchContains: '报告', patchStatus: 'doing' }],
  },
  {
  ...
  // ─── 时间字段宁缺毋滥 ─────────────────────────────────
  {
    ...
    text: '记得给老妈打电话',
    expected: [{ intent: 'ADD', taskTextContains: '老妈', hasNoExpectAt: true, hasNoDueAt: true }],
  },
  {
    name: '「最晚」型截止：dueAt 应填，expectAt 不填',
    text: '下周一前把物业费交了',
    expected: [{ intent: 'ADD', taskTextContains: '物业', hasNoExpectAt: true, hasDueAt: true }],
  },
