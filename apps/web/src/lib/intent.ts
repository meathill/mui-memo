 ...
 */
export function pickProvider(env: IntentEnv, country?: string | null): 'openai' | 'gemini' {
  if (env.AI_PROVIDER === 'openai' || env.AI_PROVIDER === 'gemini') return env.AI_PROVIDER;
...
export async function resolveAndParseVoiceIntent(env: IntentEnv, args: ParseArgs): Promise<Utterance> {
  ...
  if (provider === 'openai') {
    if (!env.OPENAI_API_KEY || !env.OPENAI_BASE_URL || !env.OPENAI_MODEL) {
      throw new Error('OpenAI 兼容端点需要 OPENAI_API_KEY / OPENAI_BASE_URL / OPENAI_MODEL');
    }
