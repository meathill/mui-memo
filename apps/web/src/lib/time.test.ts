...
describe("relativeTimeLabel", () => {
  ...
  it("过期：按分/小时/天拆档", () => {
    ...
    expect(relativeTimeLabel("2026-04-23T11:30:00+08:00", now)).toBe(
      "过期 30 分钟",
    );
    expect(relativeTimeLabel("2026-04-23T09:00:00+08:00", now)).toBe(
      "过期 3 小时",
    );
    expect(relativeTimeLabel("2026-04-21T12:00:00+08:00", now)).toBe(
      "过期 2 天",
    );
  });

  it("临近：分/小时", () => {
    expect(relativeTimeLabel("2026-04-23T12:12:00+08:00", now)).toBe(
      "12 分钟后",
    );
    expect(relativeTimeLabel("2026-04-23T15:00:00+08:00", now)).toBe(
      "3 小时后",
    );
  });
  ...
  it("跨过 24 小时：落到「明天 HH:mm」", () => {
    ...
    expect(relativeTimeLabel("2026-04-24T13:00:00+08:00", now)).toMatch(
      /^明天 /,
    );
  });
