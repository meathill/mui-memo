...
describe("rowToView", () => {
  ...
  it("tags：优先用 tags 列，为空时回退旧 tag 单标签", () => {
    expect(rowToView(makeRow({ tags: ["网银", "报销"] })).tags).toEqual([
      "网银",
      "报销",
    ]);
    expect(rowToView(makeRow({ tags: null, tag: "采购" })).tags).toEqual([
      "采购",
    ]);
...
describe("mergeTagCandidates", () => {
  it("本地候选优先，远程候选补足，并去重 trim", () => {
    expect(mergeTagCandidates([" 网银 ", "采购"], ["网银", "报销"])).toEqual([
      "网银",
      "采购",
      "报销",
    ]);
...
describe("planPersist", () => {
  it("after 里 before 没有的 → inserts，时间字段转成 Date", () => {
    const after = [
      makeView({ id: "new1", expectAt: "2026-03-01T10:00:00.000Z" }),
    ];
    ...
    expect((inserts[0].expectAt as Date).toISOString()).toBe(
