      ...
      <section className="pt-2 pb-12 sm:pb-16">
        <header className="max-w-[48rem]">
          ...
          <p className="mt-5 max-w-[36rem] text-[1.05rem] leading-[1.72] text-ink-soft sm:text-[1.18rem]">
            个人记事免费够用。AI 用得猛升级 Pro。Team
            先不作为当前主推，等个人版稳定后再开放候补。
          </p>
        </header>
      </section>
      ...
      <section className="border-t border-rule/60 pt-12 pb-16 sm:pt-14">
        <div className="max-w-[48rem]">
          ...
          <p className="mt-3 text-[1rem] leading-[1.78] text-ink-soft sm:text-[1.06rem]">
            每次按住麦克风说一句话、由 AI 解析后生成 / 修改 / 完成任务，算 1
            次。手动勾选完成、手动改字段不算。 一句话拆成多个任务也只算 1
            次。120 次 ≈ 每天 4 句，普通用户够用。
          </p>
        </div>
        ...
        <div className="mt-10 max-w-[48rem]">
          ...
          <p className="mt-3 text-[1rem] leading-[1.78] text-ink-soft sm:text-[1.06rem]">
            当月剩余的 AI
            操作用完，下次按麦克风会弹升级提示。任务管理、云同步、查看历史不受影响——你已经记下的事不会消失。
            额度每月 1 号清零重置，等到 1 号又会回来 120 次。
          </p>
        </div>
        ...
        <div className="mt-10 max-w-[48rem]">
          ...
          <p className="mt-3 text-[1rem] leading-[1.78] text-ink-soft sm:text-[1.06rem]">
            订阅在 App Store / Apple ID
            设置里随时取消，到期后自动降回免费。退款规则按 Apple
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 flex items-baseline gap-1">
        <span className="font-serif text-[2.6rem] leading-none text-ink">
          {tier.price}
        </span>
        {tier.priceSub ? (
          <span className="text-[0.95rem] text-ink-soft">{tier.priceSub}</span>
        ) : null}
      </div>
      {tier.altPrice ? (
        <p className="mt-1 text-[0.85rem] text-ink-mute">{tier.altPrice}</p>
      ) : null}
