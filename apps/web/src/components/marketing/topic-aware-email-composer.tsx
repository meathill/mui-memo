...
const TOPIC_PRESETS: Record<string, Preset> = {
  ...
  bug: {
    eyebrow: 'Bug 反馈',
    title: '报告一个具体的 bug',
    description: '把当时说的话、期望的结果、实际发生了什么、设备 / 浏览器 / iOS 版本写清楚，复现概率会大大增加。',
    subject: '叨叨记 Bug 反馈',
    body:
      '你好 meathill，\n\n' +
      '遇到了一个 bug：\n\n' +
...
export function TopicAwareEmailComposer() {
  ...
  return (
    <aside className="not-prose mb-10 rounded-2xl border border-accent-warm/40 bg-accent-warm/8 px-5 py-6 sm:px-7 sm:py-7">
      ...
      <h2 className="font-serif mt-3 text-[1.7rem] leading-tight text-ink sm:text-[1.85rem]">{preset.title}</h2>
