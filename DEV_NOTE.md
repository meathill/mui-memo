...
- `useAppStore` 走 `zustand/persist` + AsyncStorage：持久化 `tasks` + `theme` + `queue`（不含 `place` / ranked / recording 等瞬时态），杀 app 重开就立即看到上次列表，随后后台刷新。`place` 是「当次场景上下文」**不持久化**——每次启动回到默认「全部」，`merge` 兜掉旧安装里残留的 place
