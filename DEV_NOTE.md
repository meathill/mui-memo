...
**⚠️ `prepareToRecordAsync()` 不传参数会复用上一次的录音文件**：`useAudioRecorder()` 拿到的 recorder 跨组件生命周期只有一个实例；iOS 原生层（`expo-audio`）只有传入非 `undefined` 的 `options` 才会新建 `AVAudioRecorder` + 新文件，不传就静默复用上一次的文件——不会报错，`recorder.uri` 照样有值，但内容是最后一次写入的。[mic-button.tsx](apps/app/src/components/memo/mic-button.tsx) 每次 `prepareToRecordAsync` 必须显式传 `RecordingPresets.HIGH_QUALITY`。这个坑在录音队列化（连录多条、不等上一条处理完）之后才会暴露：连录 3 条会导致 3 个 `QueueItem.localUri` 指向同一个物理文件，谁最后写入内容，队列里还没上传的条目读到的就是谁的。
...
## pnpm 11：nodeLinker 必须写 pnpm-workspace.yaml（2026-07）

- pnpm 11 起 **不再读 `.npmrc` 的配置**（`pnpm config get node-linker` → undefined），只认 `pnpm-workspace.yaml`
- 本仓库依赖 hoisted 布局（Metro 解析 expo-router 兄弟 peer dep、expo-widgets 的 Podfile autolinking 都穿不过 `.pnpm` 隔离层）；升级 pnpm 后 install 悄悄回退 isolated，症状：`pod install` 在 expo-widgets 的 `require('expo-modules-autolinking')` 处崩、Metro 报 module not found
- 已把 `nodeLinker: hoisted` 移入 `pnpm-workspace.yaml` 并删除 `.npmrc`。以后 pnpm 大版本升级先检查布局：`ls node_modules/expo-modules-autolinking` 存在 = hoisted 正常
- 另：本机 CocoaPods（brew ruby 4.x）在非交互 shell 下缺 `LANG` 会报 `Unicode Normalization not appropriate for ASCII-8BIT`，跑 `pod install` 带上 `LANG=en_US.UTF-8` 即可

## 任务保险箱（Secret Vault，v0.6 app）

**架构（零知识）**：任务本体保持明文；`tasks.vault_key` 只存 app mint 的随机 UUID 指针。密文存自有 HSM 服务（https://hsm.meathill.com，信封加密：`PUT/GET/DELETE /keys/mui-memo/{userId}/{vaultKey}`，header `X-HSM-Secret`）；解密密钥（32 字节随机 hex = 恢复码）在设备 Keychain。App 直连 HSM，mui-memo 服务器永远接触不到明文与密钥。

- **vaultKey 独立于 taskId**：key 所有权绑定首次写入的 secret，用户换恢复码后旧路径 PUT 会 403——`saveVault` 遇 403 自动 mint 新 UUID 重试，旧条目沦为不可读垃圾
- **HSM 实测协议与文档的偏差（2026-07 curl 验证，勿凭文档改 client）**：响应是 `{success, data:{path, value}}` envelope（值在 `data.value`，不是顶层）；PUT 新建返回 201；PUT/DELETE 用错 secret → 403，但 **GET 用错 secret → 400**（KEK 派生错导致 AES-GCM 解密失败），`vault-hsm.ts` 对 400 按 error 文案含 "decrypt" 归类为 forbidden
- **AI 链路隔离**：`vaultKey` 只在 `updateTaskSchema`（手动 PATCH）出现；`actionSchema`/`intentConfirmSchema` 会 strip 未知键，validators.test.ts 有不变量测试锁死
- **iOS Keychain 同步 × 生物识别互斥**：`kSecAttrSynchronizable`（iCloud 钥匙串同步）与条目级 `SecAccessControl` 不能同用。取同步、生物识别放 App 层（expo-local-authentication 门控，密码管理器通行做法）
- **react-native-keychain v10**：iCloud 同步选项叫 `cloudSync`（旧文档叫 synchronizable），在 `BaseOptions` 里——**set 和 get 都要带**同样的 flag 才查得到；`accessible` 必须用非 `*_THIS_DEVICE_ONLY` 变体（默认 `AFTER_FIRST_UNLOCK` 即可），否则条目不进同步。TurboModule/新架构 OK（codegenConfig）
