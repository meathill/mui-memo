...
import { legacyToActions, legacyUtteranceSchema, parseUtteranceFlexible, utteranceSchema } from './validators.js';
...
describe('utteranceSchema', () => {
  it('ADD 意图最小载荷可通过，actions[0] 字段取默认值', () => {
    ...
    expect(parsed.actions[0].task?.place).toBeUndefined();
    expect(parsed.actions[0].task?.window).toBeUndefined();
  });
  ...
  it('dueAt 接受合法 ISO，拒绝非法字符串', () => {
    const ok = utteranceSchema.safeParse({
      raw: 'x',
      actions: [{ intent: 'ADD', task: { text: '打电话', dueAt: '2026-04-23T15:00:00+08:00' } }],
    });
    expect(ok.success).toBe(true);
    const bad = utteranceSchema.safeParse({
      raw: 'x',
      actions: [{ intent: 'ADD', task: { text: '打电话', dueAt: 'not-a-date' } }],
    });
  ...
  it('STATUS 的 patch 可以单独带 status', () => {
    ...
    if (parsed.actions[0].intent !== 'STATUS') throw new Error('expected STATUS');
...
describe('legacyToActions', () => {
  ...
  it('ADD 但无 task 字段：用 raw 兜底成 task.text', () => {
    const legacy = legacyUtteranceSchema.parse({ raw: '随便记一条', intent: 'ADD' });
    const u = legacyToActions(legacy);
