 ...
 * - workday 工作日（周一~周五）
 */
export const recurrenceFreqEnum = z.enum([
	"daily",
	"weekly",
	"monthly",
	"workday",
...
const taskPatchSchema = taskCoreSchema
	.partial()
	.extend({ status: taskStatusEnum.optional() });
	...
	switch (legacy.intent) {
		case "ADD":
			action = {
				intent: "ADD",
				...base,
				task: legacy.task ?? { text: legacy.raw },
			};
			break;
		case "STATUS":
			action = {
		...
		case "DONE":
			action = {
				...
				...(legacy.match ? { match: legacy.match } : {}),
				...(legacy.matchId ? { matchId: legacy.matchId } : {}),
				...(legacy.createIfMissing
					? { createIfMissing: legacy.createIfMissing }
					: {}),
			};
			break;
		case "MODIFY":
