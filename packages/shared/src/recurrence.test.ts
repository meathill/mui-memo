...
function inst(
	partial: Partial<RecurrenceInstanceLite> & {
		id: string;
		recurrenceId: string;
		periodIndex: number;
	},
...
describe("currentPeriodIndex", () => {
	...
	it("now 早于 anchor → null（还没开始）", () => {
		expect(
			currentPeriodIndex(anchor, new Date("2026-03-25T09:00:00Z"), "weekly", 1),
		).toBeNull();
	});
	it("now == anchor → 0", () => {
	...
	it("周期中段 → k", () => {
		expect(
			currentPeriodIndex(anchor, new Date("2026-04-11T21:00:00Z"), "weekly", 1),
		).toBe(1);
	});
	it("恰好整周期边界 → 进位到下一期", () => {
		expect(
			currentPeriodIndex(anchor, new Date("2026-04-08T09:00:00Z"), "weekly", 1),
		).toBe(1);
	});
	it("每两周：第 8 天仍是第 0 期", () => {
		expect(
			currentPeriodIndex(anchor, new Date("2026-04-09T09:00:00Z"), "weekly", 2),
		).toBe(0);
	});
	it("每两周：第 15 天进入第 1 期", () => {
		expect(
			currentPeriodIndex(anchor, new Date("2026-04-16T09:00:00Z"), "weekly", 2),
		).toBe(1);
	});
});
...
describe("periodStart", () => {
	...
	it("第 3 期起点 = anchor + 21 天", () => {
		expect(periodStart(anchor, 3, "weekly", 1).toISOString()).toBe(
...
describe("reconcileRecurrences", () => {
	it("无定义 → 空计划", () => {
		expect(reconcileRecurrences([], [], FIXED_NOW)).toEqual({
			toCreate: [],
			toDelete: [],
		});
	});
	...
	it("跳过多期：只生成当前期，删掉所有过期未完成", () => {
		const plan = reconcileRecurrences(
			...
			[
				...
				inst({ id: "t1", recurrenceId: "r1", periodIndex: 1 }),
			],
			FIXED_NOW,
		);
		expect(plan.toCreate.map((c) => c.periodIndex)).toEqual([3]);
	...
	it("未来锚点 → 不生成不删", () => {
		const plan = reconcileRecurrences(
			[def({ id: "r1", anchorAt: "2026-05-01T09:00:00Z" })],
			[],
			FIXED_NOW,
		);
	...
	it("每两周：当前期 k=1，删第 0 期未完成", () => {
		...
		expect(plan.toCreate[0]).toMatchObject({
			periodIndex: 1,
			expectAt: "2026-04-15T09:00:00.000Z",
		});
...
describe("每月 monthly", () => {
	...
	it("号数未到当月锚点日 → 仍算上一期", () => {
		...
		expect(
			currentPeriodIndex(a, new Date("2026-06-10T00:00:00Z"), "monthly", 1, 0),
		).toBe(1);
	});
	...
	it("生成当前期并清掉上月未完成", () => {
		...
		expect(plan.toCreate[0]).toMatchObject({
			periodIndex: 2,
			expectAt: "2026-06-01T09:00:00.000Z",
		});
	...
	it("按本地时区切分：UTC+8 把跨 UTC 午夜的锚点算进本地月", () => {
		...
		// 第 0 期起点就是锚点本身
		expect(periodStart(a, 0, "monthly", 1, -480).toISOString()).toBe(
...
describe("工作日 workday", () => {
	...
	it("周五（第 4 期）生成", () => {
		...
		expect(plan.toCreate[0]).toMatchObject({
			periodIndex: 4,
			expectAt: "2026-04-10T09:00:00.000Z",
		});
	});
	...
	it("按本地时区判周末：UTC 周日但本地（UTC+8）周一应生成", () => {
		...
		const utc = reconcileRecurrences(
			[def({ id: "r", freq: "workday", anchorAt, tzOffset: 0 })],
			[],
			now,
		);
		expect(utc.toCreate).toHaveLength(0); // UTC 判周日 → 跳过
		const local = reconcileRecurrences(
			[def({ id: "r", freq: "workday", anchorAt, tzOffset: -480 })],
			[],
			now,
		);
