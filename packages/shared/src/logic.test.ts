...
function task(
	partial: Partial<TaskView> & { id: string; text: string },
): TaskView {
	return {
		place: "any",
		window: "today",
...
function utter(
	partial: ActionPartial & { raw?: string; dims?: Dim[] },
): Utterance {
	return {
		raw: partial.raw ?? "",
	...
	it("回归朋友的 bug：两个独立任务不会被合并成 MODIFY", () => {
		...
		const before: TaskView[] = [
			task({
				id: "1",
				text: "提供货单给高老师",
				expectAt: "2026-04-23T10:00:00+08:00",
			}),
		];
		const u = utterMulti(
			[{ intent: "ADD", task: { text: "提供快递单号" }, aiVerb: "新增" }],
...
describe("normalizeBarChips", () => {
	...
	it("非数组 → fallback", () => {
		expect(normalizeBarChips({ kind: "place", place: "home" }, fallback)).toBe(
			fallback,
		);
