...
export function rerank(
	...
	const list = tasks.filter(
		(t) => t.status !== "linked" && (!t.done || Boolean(t.recurrenceId)),
	);
	// ctxPlace='any' = 「全部」tab：不做场景过滤，所有任务都视为「这里可做」
	const canDoHere = (t: TaskView) =>
		ctxPlace === "any" || t.place === "any" || t.place === ctxPlace;
...
function applyAction(
	...
	if (action.intent === "STATUS") {
		...
		const next = tasks.map((x) => {
			...
			if (x.status === "doing")
				return { ...x, status: "pending" as TaskStatus };
			return x;
		});
		return {
			tasks: next,
	...
	if (action.intent === "LINK") {
		...
		const next = tasks.map((x) => {
			if (x.id === doing.id) {
				return {
					...x,
					linked: [...(x.linked ?? []), { id: t.id, text: t.text }],
				};
			}
			if (x.id === t.id) {
...
function isValidBarChip(c: unknown): c is BarChip {
	...
	if (chip.kind === "place")
		return (
			typeof chip.place === "string" &&
			(PLACES as string[]).includes(chip.place)
		);
	if (chip.kind === "tag")
		return typeof chip.tag === "string" && chip.tag.length > 0;
	return false;
}
 ...
 * 防止旧装机 / 损坏存储把筛选栏搞坏。
 */
export function normalizeBarChips(
	persisted: unknown,
	fallback: BarChip[],
): BarChip[] {
